...
export async function getAutoSearchResults(searchText: string, signal: any): Promise<SearchResult[]> {
  const response = await fetch(
      `https://ac.search.naver.com/nx/ac?q=${encodeURIComponent(searchText)}&con=1&frm=nv&ans=2&r_format=json&r_enc=UTF-8&r_unicode=0&t_koreng=1&run=2&rev=4&q_enc=UTF-8&st=100`,
    {
      method: "get",
      signal: signal,
      headers: {
  ...
  const items: string[][] = json["items"][0];  // array of array

  items.map((item: string[]) => {
    results.push({
      id: nanoid(),
      query: item[0],
      description: `Search Naver for '${item[0]}'`,
      url: `https://search.naver.com/search.naver?query=${encodeURIComponent(item[0])}`,
    });
  });
